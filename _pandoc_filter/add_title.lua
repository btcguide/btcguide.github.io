...
function filter_content(content)
	...
	content = string.gsub(content, '%(setup-computer/bitcoin-node%)', '(#bitcoin-node)')
	content = string.gsub(content, '%(/running-bitcoin%)', '(#running-bitcoin)')
	content = string.gsub(content, '%(/known-issues/verify-receive-address%)', '(#verify-receive-address)')
	...
	content = string.gsub(content, '%(../verify-receive-address/cobo%)', '(#verify-receive-address-on-cobo)')
	...
	content = string.gsub(content, '%(/backup-wallet/seeds%)', '(#seeds)')
	content = string.gsub(content, '%(/setup-wallets/%)', '(#setup-wallets)')
